...
const loadFilteredDays = async (options: DayFilterOptions = {}) => {
  const notesFile = await loadNotesFile(getNotesFilePath())
  return {
    ...notesFile,
  ...
  server.registerTool(
    ...
    async (args) =>
      runTool(async () => {
        ...
        const notesFile = await loadFilteredDays({
  ...
  server.registerTool(
    ...
    async (args) =>
      runTool(async () => {
        ...
        const maxContentChars = clampLimit(args.max_content_chars, DEFAULT_CONTENT_CHARS, MAX_CONTENT_CHARS)
        const notesFile = await loadFilteredDays({ includeEmpty: false })
        const days = notesFile.days.slice(0, limit)
